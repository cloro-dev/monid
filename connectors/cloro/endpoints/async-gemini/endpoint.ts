...
import { zGeminiBody } from "../gemini/schema/inputs.ts";

/**
 * `POST /v1/async/task` with `taskType: GEMINI` — the async twin of
 * cloro#monitor/gemini. The protocol and the billing claim are described in
 * ../async-chatgpt/endpoint.ts.
 */
export default defineEndpoint({
    endpoint: "/async/gemini",
    meta: {
        displayName: "cloro Gemini (async)",
        summary:
            "Get Google Gemini's answer and cited sources for a prompt, in a chosen country, as a queued task.",
        description:
            "Run a prompt on Google Gemini, as a real user in a chosen " +
            ...
            "Gemini mentions or cites a brand, a product or a page. For " +
            "Google's search-integrated answer use cloro#monitor/aimode; " +
            "for the Google SERP AI Overview use cloro#monitor/google.",
        docsUrl:
            "https://cloro.dev/docs/api-reference/endpoint/create-async-task",
        categories: ["ai-search", "geo"],
        notes: [
            "Billing: 4 credits per request, plus 2 credits when state is set.",
        ],
    },
    request: { method: "POST", path: "/async/task" },
    input: { schema: { body: zGeminiBody } },
