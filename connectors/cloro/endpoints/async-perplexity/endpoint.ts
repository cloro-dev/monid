...
import { zPerplexityBody } from "../perplexity/schema/inputs.ts";

/**
 * `POST /v1/async/task` with `taskType: PERPLEXITY` — the async twin of
 * cloro#monitor/perplexity. The protocol and the billing claim are described in
 * ../async-chatgpt/endpoint.ts.
 */
export default defineEndpoint({
    endpoint: "/async/perplexity",
    meta: {
        displayName: "cloro Perplexity (async)",
        summary:
            "Get Perplexity's answer and cited sources for a prompt, in a chosen country, as a queued task.",
